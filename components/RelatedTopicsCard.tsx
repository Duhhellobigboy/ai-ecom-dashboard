          ...
          return (
            <li key={i} className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {type && (
                  <span className="text-xs text-zinc-600 shrink-0">{type}</span>
                )}
              </div>
              {isBreakout ? (
                <span className="text-xs px-1.5 py-0.5 rounded bg-cyan-900/40 border border-cyan-800/40 text-cyan-400 shrink-0">
                  Breakout
                </span>
              ) : (
                <span className="text-xs font-mono text-cyan-400 shrink-0">{item.value}</span>
              )}
            </li>
          );
        })}
...
export default function RelatedTopicsCard({ data, loading }: Props) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800/70 rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-zinc-800/60">
        <h2 className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Related Topics</h2>
      </div>
      <div className="px-5 py-4 space-y-4">
        {hasData ? (
          ...
          <div className="py-4 text-xs text-zinc-500 text-center">No related topic data</div>
        )}
      </div>
    </div>
  );
